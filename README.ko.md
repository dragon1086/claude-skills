...
| **최소 태스크 분석** | 3차원만 분석 (유형, 규모, 특성) — 과도한 분류 방지 |
| **다중 옵션 제시** | 최대 3가지 접근법 (안전 / 빠름 / 깊이) — 강제하지 않음 |
| **역량 갭 분석** | 미설치 도구 제안 + "없어도 가능" 명시 |
| **성능 팁** | Opus 4.6 전용 힌트 (병렬화, 백그라운드, 컨텍스트 활용) |
...
### 성능 팁
- 병렬 기회: explore + web search를 동시 실행 가능
- 백그라운드 후보: 테스트 스위트 실행
